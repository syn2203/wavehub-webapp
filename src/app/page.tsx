...
const sections = {
  meetings: {
    ...
    categories: [
      { name: '产品评审会', participants: 12, status: '进行中', duration: '45分钟', type: 'active' },
...
export default function HomePage() {
  ...
  const SectionCard = ({ sectionKey, section }: { sectionKey: string, section: any }) => {
    ...
    return (
      ...
      >
        <div className={`bg-gradient-to-br ${section.color} p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300`}>
          ...
          <div className="space-y-2">
                    ...
                    {sectionKey === 'meetings' ? (
                      <>
                        ...
                        <span className={`px-2 py-1 rounded-full ${
                          category.type === 'active' ? 'bg-green-500/20 text-green-200' : 
                          category.type === 'waiting' ? 'bg-yellow-500/20 text-yellow-200' :
                      ...
                      <>
                        <Users className="w-3 h-3" />
                        <span>{category.online}/{category.members}</span>
                      </>
            ...
            {section.categories.length > 3 && (
              <div className="text-center">
                <span className="text-white/60 text-xs">还有 {section.categories.length - 3} 个分类...</span>
              </div>
            )}
          </div>
        </div>
                  ...
                  <div className="text-right">
                    {sectionKey === 'meetings' ? (
                      <div className={`px-2 py-1 rounded-full text-xs ${
                        category.type === 'active' ? 'bg-green-100 text-green-800' :
                        category.type === 'waiting' ? 'bg-yellow-100 text-yellow-800' :
                      ...
                      <div>
                        ...
                        <div className={`text-sm ${
                          category.activity === 'high' ? 'text-green-600' : 
                          category.activity === 'medium' ? 'text-yellow-600' : 'text-gray-600'
                        }`}>
                          {category.activity === 'high' ? '活跃' : 
                           category.activity === 'medium' ? '一般' : '较少'}
                        </div>
                      </div>
                    ) : sectionKey === 'education' ? (
                      ...
                      <div className={`px-2 py-1 rounded-full text-xs ${
                        category.activity === 'high' ? 'bg-green-100 text-green-800' :
                        category.activity === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {category.activity === 'high' ? '活跃' : 
                         category.activity === 'medium' ? '一般' : '较少'}
      ...
      <main className="container mx-auto px-6 py-12">
        ...
        <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Link href="/chat" className="flex flex-col items-center p-4 rounded-xl hover:bg-white/60 transition-colors group">
            ...
            <Link href="/collaboration" className="flex flex-col items-center p-4 rounded-xl hover:bg-white/60 transition-colors group">
            ...
            <Link href="/education" className="flex flex-col items-center p-4 rounded-xl hover:bg-white/60 transition-colors group">
            ...
            <Link href="/innovation" className="flex flex-col items-center p-4 rounded-xl hover:bg-white/60 transition-colors group">
