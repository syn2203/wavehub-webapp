# LiveKit 音频房间配置指南

本项目已集成 LiveKit 实时音频通信功能，支持多人语音聊天、麦克风控制、扬声器控制等功能。

## 📋 功能特性

- ✅ 实时音频通信
- ✅ 麦克风开关控制
- ✅ 扬声器静音控制
- ✅ 音频设备切换
- ✅ 音频质量选择（高清/标准/省流）
- ✅ 参与者列表显示
- ✅ 说话状态实时显示
- ✅ 音频级别可视化
- ✅ 连接状态监控
- ✅ 自动重连机制

## 🚀 快速开始

### 1. 获取 LiveKit 凭证

有两种方式获取 LiveKit 服务：

#### 方式一：使用 LiveKit Cloud（推荐用于生产环境）

1. 访问 [https://cloud.livekit.io/](https://cloud.livekit.io/)
2. 注册并登录账号
3. 创建一个新项目
4. 在项目设置中获取：
   - WebSocket URL (例如: `wss://your-project.livekit.cloud`)
   - API Key
   - API Secret

#### 方式二：本地部署 LiveKit Server（推荐用于开发环境）

```bash
# 使用 Docker 快速启动 LiveKit Server
docker run --rm -p 7880:7880 \
  -p 7881:7881 \
  -p 7882:7882/udp \
  -e LIVEKIT_KEYS="devkey: secret" \
  livekit/livekit-server \
  --dev
```

本地部署后的配置：

- WebSocket URL: `ws://localhost:7880`
- API Key: `devkey`
- API Secret: `secret`

### 2. 配置环境变量

在项目根目录创建 `.env.local` 文件：

```bash
# LiveKit 配置
LIVEKIT_URL=wss://your-project.livekit.cloud
LIVEKIT_API_KEY=your-api-key
LIVEKIT_API_SECRET=your-api-secret
```

**注意：**

- `LIVEKIT_URL` 必须以 `NEXT_PUBLIC_` 开头，因为它会在客户端使用
- `LIVEKIT_API_KEY` 和 `LIVEKIT_API_SECRET` 仅在服务端使用，不会暴露到客户端

### 3. 启动开发服务器

```bash
npm run dev
```

### 4. 测试音频房间

1. 打开浏览器访问 `http://localhost:3000/chat`
2. 点击页面右上角的"打开语音房"按钮
3. 点击"加入房间"按钮
4. 允许浏览器访问麦克风权限
5. 点击麦克风按钮开始说话

## 🎯 使用说明

### 基本操作

1. **加入房间**
   - 点击"打开语音房"按钮显示语音房间面板
   - 点击"加入房间"按钮连接到 LiveKit 服务器
   - 首次使用需要授予麦克风权限

2. **麦克风控制**
   - 蓝色麦克风图标：麦克风已开启
   - 灰色麦克风图标：麦克风已关闭
   - 点击麦克风按钮切换状态

3. **扬声器控制**
   - 绿色扬声器图标：可以听到其他人的声音
   - 灰色扬声器图标：静音所有其他人
   - 点击扬声器按钮切换状态

4. **离开房间**
   - 点击红色挂断按钮断开连接
   - 或点击"关闭语音房"按钮隐藏面板

### 高级功能

1. **切换音频设备**
   - 如果有多个麦克风，可以在下拉菜单中选择
   - 系统会自动检测可用的音频输入设备

2. **调整音频质量**
   - **省流模式**：32kbps，适合网络较差时使用
   - **标准模式**：64kbps，默认模式，平衡质量和带宽
   - **高清模式**：128kbps，适合音乐、播客等高质量场景

3. **查看参与者**
   - 参与者列表实时显示所有在线用户
   - 正在说话的用户会有蓝色高亮和动画效果
   - 可以看到每个用户的麦克风状态

## 🔧 技术架构

### 核心组件

1. **VoiceRoom 组件** (`src/components/VoiceRoom.tsx`)
   - 负责音频房间的UI和交互逻辑
   - 管理 LiveKit Room 实例
   - 处理音频设备和权限

2. **LiveKit 配置** (`src/lib/livekit.ts`)
   - 存储 LiveKit 配置常量
   - 提供工具函数（房间名称生成、验证等）
   - 定义音频质量预设

3. **Token API** (`src/app/api/livekit/token/route.ts`)
   - 生成 LiveKit 访问令牌
   - 验证请求参数
   - 授予房间权限

### 数据流

```
用户点击"加入房间"
  ↓
前端请求 /api/livekit/token
  ↓
服务端生成 JWT token
  ↓
前端使用 token 连接 LiveKit Server
  ↓
建立 WebRTC 连接
  ↓
开始音频通信
```

## 🐛 常见问题

### 1. 无法连接到房间

**问题：** 点击"加入房间"后一直显示"连接中..."

**解决方案：**

- 检查 `.env.local` 文件是否正确配置
- 确认 LiveKit Server 是否正常运行
- 检查浏览器控制台是否有错误信息
- 如果使用 LiveKit Cloud，确认 WebSocket URL 格式正确（以 `wss://` 开头）

### 2. 麦克风权限被拒绝

**问题：** 提示"麦克风权限被拒绝"

**解决方案：**

- 点击浏览器地址栏的锁图标
- 找到"麦克风"权限设置
- 选择"允许"
- 刷新页面重试

### 3. 听不到其他人的声音

**问题：** 可以看到其他人在说话，但听不到声音

**解决方案：**

- 检查扬声器按钮是否为绿色（未静音）
- 确认系统音量未静音
- 检查浏览器是否允许自动播放音频
- 尝试点击页面任意位置激活音频上下文

### 4. 音质较差或有延迟

**问题：** 音频质量不佳或延迟明显

**解决方案：**

- 切换到"省流模式"减少带宽占用
- 检查网络连接质量
- 关闭其他占用带宽的应用
- 如果使用本地 LiveKit Server，确认服务器性能充足

### 5. 开发环境 CORS 错误

**问题：** 控制台显示 CORS 相关错误

**解决方案：**

- 确认使用的是 `ws://localhost:7880` 而不是 `wss://`
- 检查 LiveKit Server 是否使用 `--dev` 参数启动
- 如果使用 LiveKit Cloud，确认域名已添加到允许列表

## 📚 API 参考

### VoiceRoom 组件 Props

```typescript
interface VoiceRoomProps {
  roomName: string; // 房间名称（唯一标识）
  participantName: string; // 参与者名称
  onDisconnect?: () => void; // 断开连接回调
}
```

### Token API 请求格式

POST /api/livekit/token

Body:
{
  "roomName": "string",           // 必需：房间名称
  "participantName": "string",    // 必需：参与者名称
  "participantMetadata": {        // 可选：参与者元数据
    "avatar": "string",
    "joinedAt": "string"
  }
}

Response:
{
  "token": "string",              // JWT token
  "url": "string",                // WebSocket URL
  "roomName": "string",
  "participantName": "string"
}
```

## 🔐 安全建议

1. **保护 API 密钥**
   - 永远不要将 `LIVEKIT_API_KEY` 和 `LIVEKIT_API_SECRET` 提交到版本控制
   - 使用环境变量存储敏感信息
   - 在生产环境使用强密码

2. **Token 验证**
   - Token 有效期设置为 24 小时
   - 可以根据需要调整 TTL
   - 建议实现用户身份验证后再生成 token

3. **房间权限**
   - 当前配置允许所有参与者发布和订阅
   - 可以根据业务需求调整权限设置
   - 考虑实现主持人/观众角色区分

## 📖 更多资源

- [LiveKit 官方文档](https://docs.livekit.io/)
- [LiveKit JavaScript SDK](https://docs.livekit.io/client-sdk-js/)
- [LiveKit Server 部署指南](https://docs.livekit.io/deploy/)
- [WebRTC 最佳实践](https://webrtc.org/getting-started/overview)

## 🤝 支持

如有问题或建议，请：

1. 查看本文档的常见问题部分
2. 查阅 LiveKit 官方文档
3. 检查浏览器控制台错误信息
4. 联系开发团队

---

**注意：** 本项目使用的是 LiveKit 开源版本，商业使用请遵守相关许可协议。
